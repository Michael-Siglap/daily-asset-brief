...
const rateCache: Map<string, { rates: Record<string, number>; fetchedAt: number }> = new Map();
...
export async function GET(req: NextRequest) {
  ...
  if (cached && Date.now() - cached.fetchedAt < CACHE_TTL) {
    return NextResponse.json({ rate: cached.rates[to] ?? 1 }, {
  ...
  try {
    ...
    return NextResponse.json({ rate: rates[to] ?? 1 }, {
      headers: { "Cache-Control": "public, max-age=3600" },
    });
  } catch (err) {
