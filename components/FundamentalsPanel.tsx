...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between py-2 border-b border-zinc-800 dark:border-zinc-800 light:border-gray-100 last:border-0">
      <span className="text-zinc-400 dark:text-zinc-400 light:text-gray-500 text-sm">
        {label}
      </span>
      <span className="text-white dark:text-white light:text-gray-900 text-sm font-mono font-medium">
        {value}
      </span>
    </div>
  );
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-zinc-950 dark:bg-zinc-950 light:bg-white border-b border-zinc-800 dark:border-zinc-800 light:border-gray-200 px-5 py-4 flex justify-between items-center">
          <div>
            <h2 className="text-white dark:text-white light:text-gray-900 font-bold text-lg">
              {symbol}
            </h2>
            {data?.sector && (
              <p className="text-zinc-400 dark:text-zinc-400 light:text-gray-500 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="text-zinc-400 dark:text-zinc-400 light:text-gray-400 hover:text-white text-2xl leading-none"
            >
              ×
            </button>
          </div>
        ...
        {loading ? (
          ...
          <div className="px-5 py-3">
            ...
            <section className="mb-4">
              <h3 className="text-zinc-500 dark:text-zinc-500 light:text-gray-400 text-xs uppercase tracking-wider mb-2">
                Chart
              </h3>
              ...
              <section className="mb-4">
                ...
                {data.dividendRate != null && (
                  <Row
                    ...
                    value={`$${formatNumber(data.dividendRate)}`}
                  />
                )}
                {data.dividendYield != null && (
                  <Row
                    label="Dividend Yield"
                    value={`${formatNumber(data.dividendYield)}%`}
                  />
                )}
                {data.exDividendDate && (
                ...
                {data.payoutRatio != null && (
                  <Row
                    label="Payout Ratio"
                    value={`${formatNumber(data.payoutRatio)}%`}
                  />
                )}
              </section>
            )}
            ...
            <section className="mb-4">
              ...
              {data.peRatio != null && (
                <Row label="P/E Ratio" value={formatNumber(data.peRatio)} />
              )}
              {data.pbRatio != null && (
                <Row label="Price / Book" value={formatNumber(data.pbRatio)} />
              )}
              {data.epsTrailing != null && (
                <Row
                  label="EPS (TTM)"
                  value={formatCurrency(data.epsTrailing)}
                />
              )}
            </section>
            ...
            <section className="mb-4">
              ...
              {data.debtToEquity != null && (
                <Row
                  label="Debt / Equity"
                  value={formatNumber(data.debtToEquity)}
                />
              )}
              {data.totalCash != null && (
                <Row
                  label="Total Cash"
                  value={formatCurrency(data.totalCash, true)}
                />
              )}
              {data.totalDebt != null && (
                <Row
                  label="Total Debt"
                  value={formatCurrency(data.totalDebt, true)}
                />
              )}
              {data.freeCashflow != null && (
                <Row
                  label="Free Cash Flow"
                  value={formatCurrency(data.freeCashflow, true)}
                />
              )}
            </section>
            ...
            <section className="mb-4">
              ...
              {data.profitMargins != null && (
                <Row
                  label="Profit Margin"
                  value={`${formatNumber(data.profitMargins)}%`}
                />
              )}
            ...
            {data.description && (
              <section className="mb-4">
                ...
                <p className="text-zinc-400 dark:text-zinc-400 light:text-gray-600 text-xs leading-relaxed line-clamp-6">
                  {data.description}
                </p>
              </section>
            )}
